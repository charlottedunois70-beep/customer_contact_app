...
from langchain_community.utilities import SerpAPIWrapper  # ✅ 修正
from langchain.chains import LLMChain  # ✅ 修正
...
def initialize_agent_executor():
    ...
    st.session_state.hr_doc_chain = utils.create_rag_chain(ct.DB_HR_PATH)          #  新規：人事関連DB
    st.session_state.faq_doc_chain = utils.create_rag_chain(ct.DB_FAQ_PATH)        #  新規：社内FAQ
    st.session_state.manual_doc_chain = utils.create_rag_chain(ct.DB_MANUAL_PATH)  #  新規：操作マニュアル
    ...
    # 人事・従業員情報検索用のTool
    Tool(
        name=ct.SEARCH_HR_INFO_TOOL_NAME,
        func=utils.run_hr_doc_chain,
        description=ct.SEARCH_HR_INFO_TOOL_DESCRIPTION
    ),
    # 社内FAQ検索用のTool
    Tool(
